...
# Descripción: Proceso para detectar plagas en la hoja de la vid utilizando una red DenseNet201
...
# -----------------------------------------------------------------------------------------------
#
LOG_DIR=directory_log + 'DenseNet201_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
#
# -----------------------------------------------------------------------------------------------
# Función para crear el modelo DenseNet201
# -----------------------------------------------------------------------------------------------
#
def create_DenseNet201():
    model = DenseNet201(include_top=True,       # Incluir las 3 capas totalmente conectadas en la parte superior de la red
...
# -----------------------------------------------------------------------------------------------
#
DenseNet201_model.compile(loss='categorical_crossentropy',    # Función de pérdida
...
# -----------------------------------------------------------------------------------------------
#
tf.keras.utils.plot_model(model=DenseNet201_model,
                       to_file=directory_log + 'modelos/' + 'Plotmodel_DenseNet201.png',
                       show_shapes=True,
...
# -----------------------------------------------------------------------------------------------
#
DenseNet201 = DenseNet201_model.fit(x_train,                        # Imágenes de entrenamiento
...
plt.ylabel("Accuracy")  
plt.title("Training Accuracy vs Validation Accuracy en DenseNet201")  
plt.legend(['train','validation'])
plt.savefig(directory_log + 'modelos/' + 'TrainingAccuracyvsValidationAccuracy_en_DenseNet201.png')  
...
plt.xlabel("Num of Epochs")  
plt.ylabel("Loss")  
plt.title("Training Loss vs Validation Loss en DenseNet201")  
plt.legend(['train','validation'])
plt.savefig(directory_log + 'modelos/' + 'TrainingLossvsValidationLoss_en_DenseNet201.png')  

#                               Training mse vs Validation mse
plt.figure(2)  
plt.plot(DenseNet201.history['mse'],'r')  
plt.plot(DenseNet201.history['val_mse'],'g')  
plt.xticks(np.arange(0, EPOCHS, INTERVALO))
plt.rcParams['figure.figsize'] = (8, 6)  
plt.xlabel("Num of Epochs")  
plt.ylabel("MSE")  
plt.title("Training mse vs Validation mse en DenseNet201")  
plt.legend(['train','validation'])
plt.savefig(directory_log + 'modelos/' + 'TrainingmsevsValidationmse_en_DenseNet201.png')  
#
plt.show()
#
# -----------------------------------------------------------------------------------------------
...
print("[INFO]",
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "Calculando la precisión del modelo DenseNet201... ")
#
resultado_test = DenseNet201_model.evaluate(x_test,
                              y_test_one_hot)
#
print("[INFO]",
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    f"Test Accuracy DenseNet201: {resultado_test[1]*100}")
#
print("[INFO]",
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    f"Test Loss DenseNet201: {resultado_test[0]*100}")  # no sé si el loss se multiplica o no
#
# -----------------------------------------------------------------------------------------------
# Genera las predicciones de salida para el conjunto de datos test
# -----------------------------------------------------------------------------------------------
#
predicted_classes_test = DenseNet201_model.predict(x_test)
#
predicted_classes=[]
...
for i, correct in enumerate(correct[0:9]):
    ...
    plt.tight_layout()
#
plt.savefig(directory_log + 'modelos/' + 'Ejemploprediccionescorrectas_en_DenseNet201.png')  
plt.show()  
#
# -----------------------------------------------------------------------------------------------
...
for i, incorrect in enumerate(incorrect[0:9]):
    ...
    plt.tight_layout()
#
plt.savefig(directory_log + 'modelos/' + 'Ejemploprediccionesincorrectas_en_DenseNet201.png')  
plt.show()  
#
# -----------------------------------------------------------------------------------------------
...
print("[INFO]",
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "Creando la matriz de confusión DenseNet201...")
#
# -----------------------------------------------------------------------------------------------
...
plt.title('Matriz de confusión DenseNet201', pad=100, fontsize = 30, color='Black', fontstyle='italic')
plt.savefig(directory_log + 'modelos/' + 'Matrizdeconfusion_en_DenseNet201.png')  
...
#
TFGVOP_save_model.save_model(directory_log,'DenseNet201',DenseNet201)
