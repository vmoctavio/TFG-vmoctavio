...
if v_argumentos["modelo"] not in MODELS.keys():
    print("[INFO]", 
        datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "El argumento --modelo debe tener uno de los modelos válido: vgg16, vgg19, DenseNet201, Inception, ResNetV2, resnet")
    quit()
#
print("[INFO]", 
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "Modelo: ",v_argumentos["modelo"])
#
# -----------------------------------------------------------------------------------------------
...
loss_function='categorical_crossentropy'       # Función de pérdida   es la que tiene sentido. tb se usa to_categorical
...
tf.keras.utils.plot_model(model=network,
                        to_file=directory_log + 'modelos/' + 'Plotmodel_' + v_argumentos["modelo"] + '.png',
                        show_shapes=True,
...
plt.savefig(directory_log + 'modelos/' + 'TrainingAccuracyvsValidationAccuracy_en_' + v_argumentos["modelo"] + '.png')  
...
plt.legend(['train','validation'])
plt.savefig(directory_log + 'modelos/' + 'TrainingLossvsValidationLoss_en_' + v_argumentos["modelo"] + '.png')  
...
plt.legend(['train','validation'])
plt.savefig(directory_log + 'modelos/' + 'TrainingmsevsValidationmse_en_' + v_argumentos["modelo"] + '.png')  
#
plt.show()
#
# -----------------------------------------------------------------------------------------------
...
resultado_test = network.evaluate(x_test,
                              y_test_one_hot)
#
print("[INFO]",      # no sé cuál de las dos opciones .... aqui
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    f"Test Accuracy {nombre_modelo}: {resultado_test[1]*100}")
#TFGVOP_train_model.py
print("[INFO]",
    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    f"Test Loss {nombre_modelo}: {resultado_test[0]*100}")  # no sé si el loss se multiplica o no
#
# -----------------------------------------------------------------------------------------------
...
for i, correct in enumerate(correct[0:9]):
    ...
    plt.tight_layout()
#
plt.savefig(directory_log + 'modelos/' + 'Ejemploprediccionescorrectas_en_' + v_argumentos["modelo"] + '.png')  
plt.show()  
#
# -----------------------------------------------------------------------------------------------
...
for i, incorrect in enumerate(incorrect[0:9]):
    ...
    plt.tight_layout()
#
plt.savefig(directory_log + 'modelos/' + 'Ejemploprediccionesincorrectas_en_' + v_argumentos["modelo"] + '.png')  
plt.show()  
#
# -----------------------------------------------------------------------------------------------
...
plt.savefig(directory_log + 'modelos/' + 'Matrizdeconfusion_en_' + v_argumentos["modelo"] + '.png')  
...
#
TFGVOP_save_model.save_model(directory_log,v_argumentos["modelo"],execution_network)
